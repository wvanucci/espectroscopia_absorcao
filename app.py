...
@st.cache_data
def gerar_grafico_etapa1(pico_slider, x_min_slider, x_max_slider):
    ...
    if x_min_slider <= pico_slider <= x_max_slider:
        ax.axvline(x=pico_slider, color='gold', linestyle='--', linewidth=2, label=f'Pico de Emissão (λ_pico): {pico_slider:.1f} nm')
    ax.set_title(f"Espectro para λ de radiação máxima = {pico_slider} nm (Classe {classe})", color='white', fontsize=16)
...
@st.cache_data
def gerar_grafico_etapa3(temp_e3, log_g_e3, x_min_e3, x_max_e3):
...
@st.cache_data
def gerar_grafico_etapa4(temp_e4, log_g_e4, x_min_e4, x_max_e4):
...
@st.cache_data
def gerar_grafico_etapa5(temp_e5, log_g_e5, x_min_e5, x_max_e5, marcadores_selecionados):
...
with tab1:
    ...
    pico_slider = st.slider("Pico de Emissão (λ_pico):", 50, 3000, 500, 5, key="pico_e1")
    c1, c2 = st.columns(2)
    with c1:
        x_min_slider = st.slider("Escala horizontal $\lambda_{mín}$ (nm):", 0, 2000, 0, 10, key="zoom_min_e1")
    with c2:
...
with tab2:
    ...
    # Lógica de plotagem da Etapa 2 (agora com cache)
    fig_atlas = gerar_grafico_etapa2()
    st.pyplot(fig_atlas)
...
with tab4:
    ...
    **Efeito no Espectro:** Uma gravidade maior (log g alto) comprime a atmosfera estelar, causando mais colisões entre os átomos. Essa "pressão" alarga as linhas de absorção. Portanto, estrelas gigantes (log g baixo) têm linhas espectrais finas e nítidas, enquanto estrelas anãs (log g alto) têm linhas mais largas.
    """)
    # Lógica de plotagem da Etapa 4 (agora com cache)
    fig4 = gerar_grafico_etapa4(temp_e4, log_g_e4, x_min_e4, x_max_e4)
    st.pyplot(fig4)
...
with tab5:
    ...
    with c2:
        x_min_e5 = st.slider("Escala horizontal $\lambda_{mín}$ (nm):", 100, 1000, 380, 10, key="zoom_min_e5")
        x_max_e5 = st.slider("Escala horizontal $\lambda_{máx}$ (nm):", 400, 2000, 750, 10, key="zoom_max_e5")
